...
1. **Een centraal compliance dashboard** waar gemeenten hun risico's, maatregelen en audits beheren volgens de 'In Control' aanpak (Mitigeren / Zekerheid verkrijgen / Monitoren / Accepteren) — met voor elke norm (BIO, ISO 27001, AVG) inzicht in waar je staat.
...
### 2.4 Design Principes

| Principe | Betekenis |
|----------|-----------|
| **Model First** | Eerst het datamodel bouwen, dan pas de schermen |
| **Strict Separation** | API weet niets van Frontend. Frontend weet niets van Database |
| **Local AI First** | Geen data naar externe clouds (OpenAI/Anthropic) tenzij expliciet toegestaan |
| **Supply Chain Modeling** | Scopes kunnen afhankelijkheden hebben voor ketenrisico-analyse |
| **Generative UI** | Dashboards worden dynamisch gegenereerd op basis van gebruikersvragen en opgeslagen als JSON config |
...
### 4.2 "In Control" Model

Het 'In Control' model bepaalt hoeveel **aandacht** een risico krijgt, gebaseerd op twee assen:
...
|-------|-----------|------------------|
| **Risk Agent** | In Control model, MAPGOOD, classificatie | risk_* |
...
|------------|--------------|
| **Rosetta Stone Mapping** | Automatisch voorstellen van mappings tussen frameworks (ISO 27001 ↔ BIO ↔ AVG) |
...
Voor volledige specificaties, zie:

| Document | Inhoud |
|----------|--------|
| `AI_AGENTS_IMPLEMENTATION_PLAN.md` | Alle 17 agents, tool definities, handoff flows, system prompts |
| `AI_KNOWLEDGE_IMPLEMENTATION_PLAN.md` | RAG architectuur, knowledge base structuur, embeddings, token budgeting |
| `TECHNICAL_DECISIONS.md` | AI governance model, performance optimalisatie, streaming implementatie |
...
| Entity | Beschrijving |
|--------|--------------|
| `Risk` | Risico met In Control classificatie |
...
| In Control model | Praktischer dan traditioneel, Nederlandse context |
...
| Term | Definitie |
|------|-----------|
| **Attention Quadrant** | In Control model kwadrant (Mitigate/Assurance/Monitor/Accept) |
