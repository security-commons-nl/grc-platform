...
async def update_region(region_id: UUID, data: RegionUpdate, db: AsyncSession = Depends(get_db)):
    result = await db.execute(select(Region).where(Region.id == region_id))
    region = result.scalar_one_or_none()
    if not region:
        raise HTTPException(status_code=404, detail="Region not found")
...
async def create_user_tenant_role(data: UserTenantRoleCreate, db: AsyncSession = Depends(get_db)):
...
async def delete_user_tenant_role(role_id: UUID, db: AsyncSession = Depends(get_db)):
...
async def create_user_region_role(data: UserRegionRoleCreate, db: AsyncSession = Depends(get_db)):
...
async def delete_user_region_role(role_id: UUID, db: AsyncSession = Depends(get_db)):
