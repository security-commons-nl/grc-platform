...
async def list_maturity_profiles(
    ...
    query = select(IMSMaturityProfile).where(IMSMaturityProfile.tenant_id == tenant_id)
    if domain:
...
async def get_maturity_profile(profile_id: UUID, db: AsyncSession = Depends(get_db)):
...
async def create_maturity_profile(
    ...
    profile = IMSMaturityProfile(tenant_id=tenant_id, **data.model_dump())
    db.add(profile)
    await db.commit()
...
async def update_maturity_profile(
    profile_id: UUID, data: MaturityProfileUpdate, db: AsyncSession = Depends(get_db)
):
    result = await db.execute(
        select(IMSMaturityProfile).where(IMSMaturityProfile.id == profile_id)
    )
...
async def delete_maturity_profile(profile_id: UUID, db: AsyncSession = Depends(get_db)):
    result = await db.execute(
        select(IMSMaturityProfile).where(IMSMaturityProfile.id == profile_id)
    )
    profile = result.scalar_one_or_none()
...
async def list_setup_scores(
    ...
    query = select(IMSSetupScore).where(IMSSetupScore.tenant_id == tenant_id)
    if domain:
...
async def get_setup_score(score_id: UUID, db: AsyncSession = Depends(get_db)):
...
async def create_setup_score(
    ...
    score = IMSSetupScore(tenant_id=tenant_id, **data.model_dump())
    db.add(score)
    await db.commit()
...
async def update_setup_score(score_id: UUID, data: SetupScoreUpdate, db: AsyncSession = Depends(get_db)):
    result = await db.execute(select(IMSSetupScore).where(IMSSetupScore.id == score_id))
    score = result.scalar_one_or_none()
    if not score:
        raise HTTPException(status_code=404, detail="SetupScore not found")
...
async def delete_setup_score(score_id: UUID, db: AsyncSession = Depends(get_db)):
    result = await db.execute(select(IMSSetupScore).where(IMSSetupScore.id == score_id))
    score = result.scalar_one_or_none()
    if not score:
        raise HTTPException(status_code=404, detail="SetupScore not found")
...
async def list_grc_scores(
    ...
    query = select(IMSGRCScore).where(IMSGRCScore.tenant_id == tenant_id)
    if domain:
...
async def get_grc_score(score_id: UUID, db: AsyncSession = Depends(get_db)):
...
async def create_grc_score(
    ...
    score = IMSGRCScore(tenant_id=tenant_id, **data.model_dump())
    db.add(score)
    await db.commit()
    await db.refresh(score)
    return score
...
async def update_grc_score(score_id: UUID, data: GRCScoreUpdate, db: AsyncSession = Depends(get_db)):
    result = await db.execute(select(IMSGRCScore).where(IMSGRCScore.id == score_id))
    score = result.scalar_one_or_none()
    if not score:
        raise HTTPException(status_code=404, detail="GRCScore not found")
...
async def delete_grc_score(score_id: UUID, db: AsyncSession = Depends(get_db)):
    result = await db.execute(select(IMSGRCScore).where(IMSGRCScore.id == score_id))
    score = result.scalar_one_or_none()
    if not score:
        raise HTTPException(status_code=404, detail="GRCScore not found")
